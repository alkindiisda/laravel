var App = function () {
    ...
    var handleEditableTables = function () {
        ...
        function editRow(oTable, nRow, type) {
            ...
            if(type!='normal') {
                var inner = "";
                var ctr = 0;
                for (ctr = 0; ctr < existing.length - 2; ctr++) {
                    inner += existing[ctr].outerHTML;
                }
                var saya = $(nRow).clone()[0].cells;
                // console.log(aData);
                inner += saya[ctr].outerHTML;
                inner += saya[ctr + 1].outerHTML;
                nRow.innerHTML = inner;
                // console.log(nRow.innerHTML);
            }
            var jqTds = $('>td', nRow);
            var count = 0;
            while (count < aData.length - 2) {

               // console.log(existing[count]);
                console.log(jqTds[count].abbr);
               // console.log(jqTds[count].accessKey);
                //console.log("index: "+jqTds[count].cellIndex+", value : "+jqTds[count].attributes[0].nodeValue);

                backupField.push(jqTds[count].innerHTML);
                //jqTds[count].innerHTML = '<input type="text" class="m-wrap small" value="' + aData[count] + '"'+jqTds[count].abbr+'>';
                jqTds[count].innerHTML = '<input type="text" style="width:100%" value="' + aData[count] + '"'+jqTds[count].abbr+'>';
                jqTds[count].innerHTML=generateEdit(jqTds[count].innerHTML,count);
                count++;
            }
            ...
            console.log(count);
            if(type=='normal') {
                jqTds[count].innerHTML = '<a class="edit" href="">Save</a>';
            }else{
                jqTds[count].innerHTML = '<a class="edit" href="">Insert</a>';
            }

            jqTds[count + 1].innerHTML = '<a class="cancel" href="">Cancel</a>';
        }
        ...
        function saveRow(oTable, nRow, action) {
            ...
            if (action == 'delete') {
                while (count < jqInputs.length - 2) {

                    input += "col" + count + "=" + jqInputs[count] + "&";
                    count++;
                }
                popupTitle = 'Data has been deleted!';
            } else {
                jqInputs = $('input', nRow);
                while (count < jqInputs.length) {
                    // input.push("col"+count+":"+jqInputs[count].value);
                    input += "col" + count + "=" + jqInputs[count].value + "&";
                    count++;
                }
                if (action == 'insert') {
                    popupTitle = 'Data has been inserted!';
                } else {
                    popupTitle = 'Data has been updated!';
                }
            ...
            $.ajax({
                type: 'POST',
                url: $("#sample_editable_1").attr("prefix"),
                /*data: { col0: jqInputs[0].value,col1: jqInputs[1].value,
                 col2: jqInputs[2].value,col3: jqInputs[3].value }*/
                data: input
            ...
            console.log(data[0]);
                ...
                if (action != 'delete') {
                    var ctr = 0;
                    //console.log(nRow);
                    while (ctr < jqInputs.length) {
                        console.log(data[1]['col'+ctr]);
                       // console.log(getColName[ctr].abbr);
                        console.log(typeof data[1]['col'+ctr]!='undefined');
                        if(typeof data[1]['col'+ctr]!='undefined'){
                            oTable.fnUpdate(data[1]['col'+ctr], nRow, ctr, false);
                        }else {
                            oTable.fnUpdate(jqInputs[ctr].value, nRow, ctr, false);
                        }
                        ctr++;
                    }

                    oTable.fnUpdate('<a class="edit" href="">Edit</a>', nRow, ctr, false);
                    oTable.fnUpdate('<a class="delete" href="">Delete</a>', nRow, ctr + 1, false);
                    oTable.fnDraw();
                }
                ...
                console.log(data);
                console.log('server errors', data.responseText);
                console.log(backupField);
                console.log(backupField[3]);
                if (action == 'update') {
                    var ctr = 0;
                    while (ctr < backupField.length) {
                        oTable.fnUpdate(backupField[ctr], nRow, ctr, false);
                        console.log(backupField[ctr]);
                        ctr++;
                    }
        ...
        function cancelEditRow(oTable, nRow) {
            ...
            while (count < jqInputs.length) {
                oTable.fnUpdate(jqInputs[count].value, nRow, count, false);
                count++;
            }
            ...
            oTable.fnUpdate('<a class="edit" href="">Edit</a>', nRow, count, false);
            oTable.fnDraw();
        }
        ...
        $('#sample_editable_1_new').click(function (e) {
            oTable.fnFilter("");
            if(isInserting) {
                existing=$("#sample_editable_1 > tbody tr:first td").clone().empty();
                var totalcolumn = $("#sample_editable_1 > tbody").find("tr:first td").length - 2;
                var data = [];
                while (totalcolumn--) {
                    data.push('');
                }
                data.push('<a class="edit" href="">Edit</a>');
                data.push('<a class="cancel" data-mode="new" href="">Cancel</a>');
                console.log(data);
                e.preventDefault();
        ...
        $('#sample_editable_1 a.delete').live('click', function (e) {
            ...
            var nRow = $(this).parents('tr')[0];
            var deletedData = oTable.fnDeleteRow(nRow);
            console.log(deletedData[0]._aData);
            saveRow(oTable, deletedData[0]._aData, 'delete');
        ...
        $('#sample_editable_1 a.edit').live('click', function (e) {
            ...
            }  else if (nEditing == nRow && this.innerHTML == "Insert") {


                /* Editing this row and want to save it */
                saveRow(oTable, nEditing, "insert");
                nEditing = null;
    ...
    var handleChat = function () {
        ...
        var handleClick = function () {
            ...
            tpl += '<a href="#" class="name">' + $(".username").html() + '</a>&nbsp;';
...
function showSuccess(msg){
    $(".alert-success").hide();
    $(".alert-success").text("Success!!! "+msg);
    $(".alert-success").show();
}
